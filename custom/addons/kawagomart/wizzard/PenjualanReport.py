...
class PenjualanReport(models.TransientModel):
    _name = 'kawagomart.penjualanreport'
    ...
    def action_penjualan_report(self):
        ...
        print(filter)
        penjualan = self.env['kawagomart.penjualan'].search_read(filter)
        print(penjualan)
        data = {
            'form': self.read()[0],
        ...
        print(data)
        return self.env.ref('kawagomart.wizzard_penjualanreport_pdf').report_action(self, data=data)
